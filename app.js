...
// Initialize TTS after content is rendered
function initTTS() {
    // Find all Japanese text elements that should be clickable
    ...
    japaneseSelectors.forEach(selector => {
        ...
        elements.forEach(el => {
            makeTextClickableForTTS(el);
        });
    });
}

// Make Japanese text clickable for TTS
function makeTextClickableForTTS(element) {
    // Skip if element already has click handlers or is a link
    if (element.closest('a') || element.hasAttribute('data-tts-processed')) {
        return;
    }
    
    // Mark as processed
    element.setAttribute('data-tts-processed', 'true');
    ...
    element.addEventListener('click', function(e) {
        ...
        // Get Japanese text (exclude translation elements)
        const japaneseText = getJapaneseTextOnly(this);
    ...
    element.title = 'クリックして音声を聞く / Klik untuk mendengarkan suara';
}

// Extract only Japanese text, excluding translations
function getJapaneseTextOnly(element) {
    // Clone the element to manipulate it
    const clone = element.cloneNode(true);
    
    // Remove all translation elements
    const translations = clone.querySelectorAll('.translation-en, .translation-id');
    translations.forEach(t => t.remove());
    
    // Get text content
    let text = clone.textContent || '';
    
    // Clean up - remove common non-Japanese text patterns
    text = text.replace(/p\.\d+/g, ''); // Remove page numbers
    text = text.replace(/[①②③④⑤⑥⑦⑧⑨⑩]/g, ''); // Remove circled numbers
...
// ============================================
// UPDATE EXISTING FUNCTIONS TO INCLUDE TTS
// ============================================

// Modify renderAllContent to call initTTS after rendering
...
// Modify showSection to re-initialize TTS when changing sections
const originalShowSection = showSection;
showSection = function(sectionId) {
    originalShowSection(sectionId);
    // Re-initialize TTS for new section
    setTimeout(initTTS, 100);
};
